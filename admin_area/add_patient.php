...
if (isset($_GET['add_patient'])) {
    ...
    if ($fname == "" && $lname == "" && $phone == "" && $address == "") {
        ...
        if (mysqli_num_rows($result_check) > 0) {
            ...
            $sql = "INSERT INTO `patient`(`patient_id`, `fname`, `lname`, `gender`, `address`, `phone`) VALUES ('','" . $fname . "','" . $lname . "','" . $gender . "','" . $address . "','" . $phone . "')";
            echo "\n" . $sql;
            if (mysqli_query($conn, $sql)) {
                //Thông báo nếu thành công
                $sql_id = "SELECT * FROM patient WHERE fname = '" . $fname . "' AND lname = '" . $lname . "' AND phone = '" . $phone . "' AND address = '" . $address . "'";
                echo $sql_id;
                ...
                if (mysqli_num_rows($result_id) > 0) {
                    while ($row = mysqli_fetch_assoc($result_id)) {
                        ...
                        foreach ($comorbidity as $comorbidityValue) {
                            $sql = "INSERT INTO `patient_comorbidity`(`patient_id`, `comorbidity`) VALUES ('" . $row['patient_id'] . "','" . $comorbidityValue . "');";
                            if (mysqli_query($conn, $sql)) {
                                //Thông báo nếu thành công
                        ...
                        foreach ($symptom as $symptomValue) {
                            $sql = "INSERT INTO `patient_symptom`(`patient_id`, `symptom`, `date_symptom`) VALUES ('" . $row['patient_id'] . "','" . $symptomValue . "','" . $date . "');";
                            if (mysqli_query($conn, $sql)) {
                                //Thông báo nếu thành công
                                echo 'Thêm thành công';
                                //echo "<script>alert('Add patient success')</script>";
                        ...
                        if ($symptoms_other != "") {
                            $sql = "INSERT INTO `patient_symptom`(`patient_id`, `symptom`, `date_symptom`) VALUES ('" . $row['patient_id'] . "','" . $symptoms_other . "','" . $date . "');";
                            if (mysqli_query($conn, $sql)) {
                                //Thông báo nếu thành công
                                echo 'Thêm thành công';
                                //echo "<script>alert('Add patient success')</script>";
